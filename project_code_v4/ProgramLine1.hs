...
printSigSetLine :: (VarName, [SignalCmd]) -> String
printSigSetLine (th, sigs) = lineHead ++ " = {|" ++ sigStrs ++ "|}"
    where lineHead = getSigSetStr th
          sigStrs = join "," $ map printSignal sigs


getAllSigSet :: [Variable] -> [ProcWLocal] -> [ProgStruc] -> String
getAllSigSet consts procs progs = (join "\n" strs) ++ "\n"
    where strs = map printSigSetLine $ map (getSigSet consts procs) progs
...
printSingleProgram :: [Variable] -> [Variable] -> [ProcWLocal] -> Program -> String
printSingleProgram globals consts procs prog = sigStr ++ (getAllProgramLine (writeMap', readMap') prog5)
    where prog1 = progLocalCompilation procs prog
          prog3 = assignThToProg prog1
          sigStr = getAllSigSet consts procs (flattenProgram prog3)
...
printOverallProgram (Overall (globals, consts, procs, asserts, signals, progs)) = str
    where str = join "\n\n" $ map (printSingleProgram globals consts procs) progs
...
printProgTree :: ProgramTree -> String
printProgTree (LocLeaf (id, prog, vars)) = head ++ " = compress((" ++ progStr ++ "[|" ++ synStr ++ "|]" ++ varsStr1 ++ ") \\ " ++ synStr ++ ")"
...
printProgTree (ProgLeaf (id, prog, vars)) = if length vars == 0 then res2 else res1
    where head = printProgHead (ProgLeaf (id, prog, vars))
          ...
          res2 = head ++ " = " ++ progStr
printProgTree (ProgNode (id, progs)) = head++" = compress((|| i: {"++range++"} @ ["++allowStr++"] "++progStr++") \\ chans)"
...
printProgram (Prog (name, comp, tree)) = line
...
getSigSetStr :: String -> String
getSigSetStr i = "sigSet("++i++")"
...
getTreeSigSet tree = "Union({"++getSigSetStr("i")++" | i <-{"++idsStr++"}})"
...
getAllProgramLine (writeMap, readMap) (Prog (name, comp, tree)) = join "\n\n" [syncLines, allowLines, treeLines, programLine]
    where syncLines = getAllSyncLine (writeMap, readMap) tree
          ...
          programLine = printProgram (Prog (name, comp, tree))
...
--           isRangeSet = True --- this should be changed later
--           start = 0
--           end = 1
--           rangeList = [Const 1]--if not isRangeSet then map (subsIExpr []) (fromJust $ getRangeList ditypeNumSet) else error "not rangeList"
--           func = \x -> if isConstIExpr x then getConstVal x else error "The expression cannot be reduced to a constant integer"
--           rangeList' = map func rangeList
--           finalRange = if isDefined then definedRange else if isRangeSet then [start..end] else rangeList'
